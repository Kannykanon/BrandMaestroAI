...
**3. Enforcer** — runs deterministic checks first (punctuation the brand never uses, measured writing rates, unfilled placeholders, invented quotes and contact details, copied source passages, facts dropped from the source, and — for scripts — story beats summarised away instead of told), then judges two things with the model: whether the draft is true and publishable, and separately whether it sounds like the brand. A draft that satisfies every rule and still reads like nobody fails the voice pass, which returns rewritten sentences rather than a score to chase. Below threshold, it sends the draft back with notes. Content with invented claims is never approved.
...
| 1b. Continue a series (optional) | Put the project in a **series** and it becomes its next episode, starting from the last one: same style, products, locations, sound and end card, and the same character for each speaker. Each episode leaves a short recap, and later episodes are planned with the series logline and those recaps. | You |
...
| 3. Cast, products and places | Characters with a voice each. Product photos, locations and logos are uploaded once per business (**Styles → Products & logos**), or a location is drawn from a description; a project lists what it features, and a shot shows a product or place when its line or visual names it, or when you tick it for that shot. A location reference keeps the same place across shots and episodes. On-screen characters get face photos (only after confirming the right to use them) and an approved character sheet. A style lock gives every scene one look. | You |
...
**Scene headings are read.** Nothing was reading them, so a draft shipped with "INT. UNIVERSITY CAMPUS, DAY" — a comma where this brand's scripts use an em dash, and INT. for a place that is outdoors. Both are mechanical: the separator is the brand's own habit, measured off its scripts, and no judgement is needed to know which side of a door a campus is on. Both are corrected in code rather than sent back for a revision round. Ambiguity is never guessed at — a diner, a hotel or a home is legitimately either prefix depending on which side of its door the scene is, and this corpus uses both for the same building. Continuity, a scene headed for one place whose action happens in another, needs to know where people are, so that is the judge's (`utils/screenplay.py`).
...
**No gate may spend the whole revision budget.** A deterministic finding short-circuits the enforcer the first time it fires, which is right: the finding is certain and a scoring call would be money spent to say the same thing. After two rounds it is carried into the full evaluation instead, so the writer gets everything wrong with the draft at once. It still refuses the draft and still blocks force-approval at the last iteration — carrying changes when the writer hears about a finding, never whether it counts. Without this, one unsatisfiable finding led all six rounds of a run and the voice pass was never invoked once.

**The pipeline never asks for its own notes back.** The researcher introduces each source with a banner and a note explaining how to treat it; those notes are defined in `utils/research_sections.py` and stripped from anything that reads the research back. They used to be read as story: for six consecutive rounds a screenplay was told it had failed to dramatise "Facts about the subject itself come from here. Where this and the external context below disagree, this wins."

**A retelling has to tell the story, not summarise it.** For narrative content, the draft is compared against the source beat by beat, and refused when it reaches a beat and leaves out what happens in it (`utils/coverage.py`). A script whose final act replaced four beats — men greeting a character with both hands, him asking about a student's studies, a phone number handed over — with "This encounter marks his initiation into a world of hidden power" passed every other check in the system: nothing it wrote was false, copied or off-brand. Marketing content is not asked to cover all of its research, because using a fraction of it is the job. Abstraction is also measured per section rather than over the whole piece, since a draft that starts concrete and gives up at the end averages out to fine.
...
**Rhythm is measured; the judge is forbidden from ruling on it.** Flatness — sentences that barely change length — is the part of voice a reader feels and a judge cannot measure by reading, and every oscillation this system has had was a judge trying: join these sentences, then split them, then join them. Worse, run against a live Brain the judge scored this brand's hand-written gold 7.0 and faulted it for "sentence rhythm" while rating a draft that had lost a fact above it. It is now measured as a share of the brand's own sentence-length variation and reported to the writer in words. Not the brand's *range*: that corpus sits at 3.38–3.49 and its gold at 2.7, because a hand-written retelling is tighter than a full screenplay and right to be — what separates the gold from the drafts that shipped wrongly (1.5, 1.8) is a gap, not a band.

**The voice pass is given craft, never rates.** It is asked one question — would a reader who knows this brand believe it wrote this? — against how the brand opens, closes and constructs, with the measured rates kept out of its prompt and out of its vocabulary. Handed those rates, it faulted a draft against whichever one the draft had crossed: three rounds ordering a script's sentences joined, then one ordering every sentence onto its own line, each defensible against a different statistic. A number a draft can cross is a target to oscillate around, not a voice. The rates are still measured and still enforced where they are mechanical — punctuation, placeholders, capitals — and a subject the judge raises is recorded and closed, so the next round cannot reopen it.

**Layout is not voice.** `share_single_sentence_paragraphs` sits near 99 in a screenplay corpus because screenplays are laid out one action per line. Reported as a voice rule, it produced an instruction to reformat an entire script, so it is measured and deliberately not reported.

**Nothing in the Brain is a template.** Openings and closings are described in a sentence ("a transition line, then a scene heading, then a couple of short sentences"), not as a numbered list with word counts. Printed as a list, the enforcer twice refused a draft for failing to match "the brand's specific sentence length sequence" — which was never a specification, only a description of what the brand usually does.

**A construction quoted from the corpus is removed before the writer sees it.** The synthesis is asked to describe the brand's moves abstractly; asking is not getting. It returned "the 'He realizes that…' declarative phrase" — a sentence from one of the uploaded screenplays — the enforcer told a draft to use it, and the draft wrote "He realizes that his wallet is gone". Descriptive sections are now checked against the corpus in code, the way the asset bank already was.
