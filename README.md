...
**The voice pass is bound by the measurement, so it cannot argue with itself.** It is told which way each measurement may move — and that a measurement inside the brand's own range is settled and may not be raised as a fault at all — plus what the previous round asked for, which it may not reverse. Without that, one round told a draft its sentences were choppy and rewrote five passages into longer ones, and the next round split the sentence it had just asked for back into three: eight rounds, no convergence, both verdicts individually defensible. The limits never instruct a change, because the hand-written correct script measures below its own brand corpus on sentence length, word length and abstraction — all three, deliberately.
